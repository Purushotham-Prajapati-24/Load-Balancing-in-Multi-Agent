...
We consider $M$ agents with computational speeds $s_1, s_2, \ldots, s_M$ and $N$ independent tasks with processing requirements $p_1, p_2, \ldots, p_N$. This formulation follows the standard framework established in scheduling theory \cite{brucker2007, leung2004}. The processing time of task $j$ on agent $i$ is:
...
The \emph{makespan} is $C_{\max} = \max_{i=1}^{M} L_i$. Our objective is to find $\sigma^* = \arg\min_\sigma C_{\max}(\sigma)$. This problem is equivalent to the multiprocessor scheduling problem shown to be NP-hard by Garey and Johnson \cite{garey1979}, with heuristic approaches surveyed by Ibarra and Kim \cite{ibarra1977}.
...
\textbf{Online setting.} Tasks arrive in a sequence \cite{borodin1998, pruhs2004}. Upon arrival of task $j$, the scheduler must irrevocably assign it to an agent without knowledge of future tasks.
...
The Hybrid Adaptive Scheduler (HAS) draws on both the offline sorting paradigm of Graham \cite{graham1969} and the online greedy paradigm analyzed by Sgall \cite{sgall1998} and Azar \cite{azar1998}. HAS operates in two phases controlled by a batch parameter $\alpha \in [0, 1]$.
...
Phase 1 requires $O(B \log B)$ for sorting \cite{cormen2022} plus $O(BM)$ for assignment. Phase 2 requires $O((N-B)M)$ for online assignment. Total:

\begin{equation}
    T(N, M, \alpha) = O(\alpha N \log(\alpha N) + NM)
\end{equation}

For $\alpha = 0$ (pure online), this reduces to $O(NM)$. For $\alpha = 1$ (pure offline), $O(N \log N + NM)$. This is polynomial for all $\alpha$ values, making HAS practical even for large-scale instances unlike the exponential-time optimal solutions \cite{garey1979, lenstra1990}.
...
\begin{proof}
Let $j^*$ be the task completing last in the HAS schedule, assigned to agent $i^*$. If $j^*$ was assigned in Phase 1 (batch), the LPT ordering ensures it is among the smallest tasks in the batch, yielding a tighter bound than arbitrary-order assignment \cite{graham1969, coffman1978}.

For the Phase 2 tasks, the standard List Scheduling analysis applies \cite{graham1966}: at the time of assignment, agent $i^*$ had the minimum load, so $L_{i^*} - t_{i^*j^*} \leq \frac{1}{M} \sum_{i} L_i$. The key improvement is that the Phase 1 LPT assignment provides a better ``warm start,'' reducing the load variance across agents before Phase 2 begins, analogous to the load-balancing improvement shown by Azar et al. \cite{azar1999}. Since the $\alpha N$ largest tasks (sorted by LPT) are assigned first, the load imbalance entering Phase 2 is bounded, improving the worst-case completion time by at least $\frac{\alpha}{M} \cdot p_{\max}^{\text{batch}}$.
\end{proof}
...
We implement HAS and five baseline algorithms in Python: Online Greedy (ECT) \cite{graham1966}, Offline LPT \cite{graham1969}, Random Assignment, and Round Robin. The offline optimal is computed via brute-force enumeration for small instances ($N \leq 15$) and LP lower bounds \cite{lenstra1990, williamson2011} otherwise. All experiments average 30 independent trials with randomized task sets, following the experimental methodology recommended by Pinedo \cite{pinedo2016}.

\textbf{Task distributions:} Uniform $p_j \sim U(1, 100)$, Pareto $p_j \sim \text{Pareto}(1.5)$ following the heavy-tailed models common in distributed systems \cite{mitzenmacher2001}, and adversarial instances designed to exploit online schedulers \cite{bartal1995}.

\textbf{Agent configurations:} Identical ($s_i = 1$), Uniform ($s_i \sim U(1, 10)$) as studied by Epstein and Sgall \cite{epstein2004}, and Bimodal (30\% fast at $10\times$, 70\% slow at $1\times$) modeling heterogeneous agent pools \cite{ibarra1977, korsah2013}.

\subsection{Results}

\subsubsection{Makespan Comparison (Identical Machines)}

Fig.~\ref{fig:identical} presents the makespan achieved by each algorithm on identical machines ($P||C_{\max}$ \cite{pinedo2016}) with $N=200$ tasks and $M \in \{4, 8, 16\}$ agents. Offline LPT consistently achieves the lowest makespan, confirming Graham's classical result \cite{graham1969}, while HAS variants closely match LPT performance despite processing a portion of tasks online.
